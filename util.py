...
def align_face(filepath, output_size=1024, transform_size=4096, enable_padding=True):
    ...
    """
    try:
        # Open image and get dimensions properly
        ...
        if shrink > 1:
            rsize = (int(np.rint(float(width) / shrink)), int(np.rint(float(height) / shrink)))
        ...
        pad = (
            max(int(np.floor(min(quad[:, 0]))) - border, 0),
            max(int(np.floor(min(quad[:, 1]))) - border, 0),
            max(int(np.ceil(max(quad[:, 0]))) + border - width, 0),
            max(int(np.ceil(max(quad[:, 1]))) + border - height, 0)
        )
